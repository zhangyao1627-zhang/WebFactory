...
public class LoginController extends BaseController {
    ...
    public FebsResponse register(
            ...
            @NotBlank(message = "{required}") String roleid,
            @NotBlank(message = "{required}") String factoryname,
            @NotBlank(message = "{required}") String factorydetail) throws FebsException {
        User user = userService.findByName(username);
        if (user != null) {
            throw new FebsException("该用户名已存在");
        }
        System.out.println("Get into the login functions");
        System.out.println("roleid" + roleid);
        userService.register(username, password,phone,roleid,factoryname,factorydetail);
